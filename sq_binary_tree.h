...
template <class ElemType>
void DisplayBTWithTreeShapeHelp(const SqBinaryTree<ElemType> &bt, int r, int level);
	//	操作结果：按树状形式显示以r为根的二叉树，level为层次数，可设根结点的层次数为1
template <class ElemType>
void DisplayBTWithTreeShape(const SqBinaryTree<ElemType> &bt);
	//	树状形式显示二叉树 
template <class ElemType>
...
// 操作结果：已知先序和中序序列构造二叉树
{
	...
	CreateBinaryTreeHelp<ElemType>(es, r, pre, in, 0, n - 1, 0, n - 1);
		// 由先序和中序序列构造以r为根的二叉树 
	SqBinaryTree<ElemType> bt(es, r, n);				// 生成二叉树
	delete []es;										// 释放存储空间
	return bt;
}
