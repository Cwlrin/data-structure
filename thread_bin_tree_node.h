...
// 数据成员:
	ElemType data;							// 数据成员
	ThreadBinTreeNode<ElemType> *leftChild;	// 左孩子指针成员
	ThreadBinTreeNode<ElemType> *rightChild;// 右孩子指针成员
	PointerTagType leftTag, rightTag;		// 左右标志成员

//	构造函数模板:
	ThreadBinTreeNode();					// 无参数的构造函数 模板
	ThreadBinTreeNode(const ElemType &val,	// 由数据元素值,指针及标志成员构造结点
...
// 操作结果：构造一个叶结点
{
	leftChild = rightChild = NULL;			// 指针成员为空
...
ThreadBinTreeNode<ElemType>::ThreadBinTreeNode(const ElemType &val, 
		...
		PointerTagType lTag,
		PointerTagType rTag)
// 操作结果：构造一个数据成员为val,左孩子为lChild,右孩子为rChild的结点,左标
//	志为lTag,右标志为rTag的结点
{	
	data = val;								// 数据元素值
