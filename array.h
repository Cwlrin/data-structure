// 文件路径名:e1_4\array.h 
#ifndef __ARRAY_H__											// 如果没有定义__ARRAY_H__
#define __ARRAY_H__											// 那么定义__ARRAY_H__
...
// 操作结果：求元素在顺序存储中的位置
{	
	if (!(sub0 >= 0 && sub0 < bounds[0])) throw "下标出界!";	// 抛出异常		
	int off = constants[0] * sub0;					// 初始化元素在顺序存储中的位置	
	for (int i = 1; i < dim; i++)
	{
		int sub = va_arg(va, int);					// 取出数组元素下标
		if (!(sub >= 0 && sub < bounds[i]))	throw "下标出界!"; // 抛出异常
