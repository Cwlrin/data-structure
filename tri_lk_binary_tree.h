...
#include "tri_lk_bin_tree_node.h"	// 三叉链表二叉树结点
...
//	辅助函数模板:
	TriLkBinTreeNode<ElemType> *CopyTreeHelp(const TriLkBinTreeNode<ElemType> *r);	// 复制二叉树
	void DestroyHelp(TriLkBinTreeNode<ElemType> * &r);						// 销毁以r为根二叉树
	void PreOrderHelp(const TriLkBinTreeNode<ElemType> *r, void (*visit)(const ElemType &)) const;	// 先序遍历
	void InOrderHelp(const TriLkBinTreeNode<ElemType> *r, void (*visit)(const ElemType &)) const;		// 中序遍历
	void PostOrderHelp(const TriLkBinTreeNode<ElemType> *r, void (*visit)(const ElemType &)) const;	// 后序遍历
...
// 操作结果：将以r为根的二叉树复制成新的二叉树,返回新二叉树的根
{
	...
	{	// 复制非空二叉树
		TriLkBinTreeNode<ElemType> *lChild = CopyTreeHelp(r->leftChild);	// 复制左子树
		TriLkBinTreeNode<ElemType> *rChild = CopyTreeHelp(r->rightChild);// 复制右子树
...
// 操作结果：已知先序和中序序列构造二叉树
{
	...
	CreateBinaryTreeHelp<ElemType>(r, pre, in, 0, n - 1, 0, n - 1);
		// 由先序和中序序列构造以r为根的二叉树 
	return TriLkBinaryTree<ElemType>(r);						// 返回以r为根的二叉树
}
