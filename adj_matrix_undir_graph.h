...
template <class ElemType>
void Display(const AdjMatrixUndirGraph<ElemType> &g, bool DisplayVexElem);	// 显示邻接矩阵无向图
template <class ElemType>
void Display(const AdjMatrixUndirGraph<ElemType> &g);						// 显示邻接矩阵无向图


// 无向图的邻接矩阵类模板的实现部分
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::DFS(int v, void (*visit)(const ElemType &e)) const
// 初始条件：存在图
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::DFSTraverse(void (*visit)(const ElemType &e)) const
// 初始条件：存在图
// 操作结果：对图进行深度优先遍历
{
	int v;
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::BFS(int v, void (*visit)(const ElemType &)) const
// 初始条件：存在图
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::BFSTraverse(void (*visit)(const ElemType &)) const
// 初始条件：存在图
// 操作结果：对图进行广度优先遍历
{
	int v;
...
template <class ElemType>
bool AdjMatrixUndirGraph<ElemType>::GetElem(int v, ElemType &e) const
...
template <class ElemType>
bool AdjMatrixUndirGraph<ElemType>::SetElem(int v, const ElemType &e)
...
template <class ElemType>
bool AdjMatrixUndirGraph<ElemType>::GetTag(int v) const
// 操作结果：返回顶点v的标志		 
{
	if (v < 0 || v >= vexNum) throw "v不合法!";	// 抛出异常
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::SetTag(int v, bool val) const 
// 操作结果：设置顶点v的标志为val		 
{
	if (v < 0 || v >= vexNum) throw "v不合法!";	// 抛出异常
...
template <class ElemType>
void Display(const AdjMatrixUndirGraph<ElemType> &g, bool DisplayVexElem)
// 操作结果: 显示邻接矩阵无向图
{
	int **Matrix;							// 邻接矩阵
...
template <class ElemType>
void Display(const AdjMatrixUndirGraph<ElemType> &g)
// 操作结果: 显示邻接矩阵无向图
{
	int **Matrix;							// 邻接矩阵
	Matrix = (int **)new int *[g.GetVexNum()];		// 生成邻接矩阵
