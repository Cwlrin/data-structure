...
{
// 数据成员:
	GenListNodeType tag;				// 标志成员,HEAD(0):头结点, ATOM(1):原子结构, LIST(2):表结点
	GenListNode<ElemType> *nextLink;	// 指向同一层中的下一个结点指针成员
	union
	{
		ElemType atom;					// tag=ATOM,存放原子结点的数据成员
		GenListNode<ElemType> *subLink;	// tag=LISK,存放指向子表的指针成员
	}; 
