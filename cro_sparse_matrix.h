...
// 辅助函数模板:
	...
	bool InsertHelp(const Triple<ElemType> &e);	// 插入十字链表三元组结点
...
// 抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	bool SetElem(int r, int c, const ElemType &v);	// 设置指定位置的元素值
	bool GetElem(int r, int c, ElemType &v);			// 求指定位置的元素值
...
template <class ElemType>
bool CLkSparseMatrix<ElemType>::SetElem(int r, int c, const ElemType &v)
// 操作结果：如果下标范围错,则返回false,否则返回true
{
...
template <class ElemType>
bool CLkSparseMatrix<ElemType>::GetElem(int r, int c, ElemType &v)
// 操作结果：如果下标范围错,则返回false,否则返回true,并用v返回指定位置元素值
{
	if (r > rows || c > cols || r < 1 || c < 1)
		return false;				// 下标范围错
...
template <class ElemType>
bool CLkSparseMatrix<ElemType>::InsertHelp(const Triple<ElemType> &e)
// 操作结果：如果下标范围错或三元组下标重复,则返回false,如果插入成功,则返回true
{
