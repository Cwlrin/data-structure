...
//	辅助函数模板:
	...
	void PostRootOrderHelp(const ChildSiblingTreeNode<ElemType> *r, void (*visit)(const ElemType &)) const;
		// 后根序遍历
	int NodeCountHelp(const ChildSiblingTreeNode<ElemType> *r) const;	// 返回以r为根的树的结点个数
	int HeightHelp(const ChildSiblingTreeNode<ElemType> *r) const;	// 返回以r为根的树的高
	int DegreeHelp(const ChildSiblingTreeNode<ElemType> *r) const;	// 返回以r为根的树的度
...
//  树方法声明及重载编译系统默认方法声明:
	...
	int NodeDegree(const ChildSiblingTreeNode<ElemType> *cur) const;	// 返回结点cur的度
	...
	bool InsertChild(ChildSiblingTreeNode<ElemType> *cur, int i, const ElemType &e);	
		// 将数据元素插入为cur的第i个孩子
	bool DeleteChild(ChildSiblingTreeNode<ElemType> *cur, int i);// 删除cur的第i个棵子树
...
{
	PreRootOrderHelp(GetRoot(), visit);				// 调用辅助函数模板实现树的先根遍历
}
...
{
	PostRootOrderHelp(GetRoot(), visit);			// 调用辅助函数模板实现后根序遍历
}
...
// 操作结果：返回以r为根的树, 结点cur的双亲
{
	...
	for (child = FirstChild(r); child != NULL;
		child = RightSibling(child))
	{	// 找查r的孩子
		if (child == cur) return (ChildSiblingTreeNode<ElemType> *)r;				// cur双亲为r
	}
...
// 操作结果：删除cur的第i个棵子树,如果删除成功,则返回true,否则返回false
{
	if (cur == NULL) return false;								// cur不存在
...
// 操作结果：按凹入表示法显示树
{
	DisplayTWithConcaveShapeHelp(t, t.GetRoot(), 1);// 调用辅助函数模板实现按凹入表示法显示树
	cout << endl;									// 换行
}
