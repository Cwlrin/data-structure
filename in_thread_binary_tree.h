...
//  线索二叉树方法声明及重载编译系统默认方法声明:
	...
	virtual ~InThreadBinTree();								// 析构函数模板
	...
	InThreadBinTree(const InThreadBinTree<ElemType> &copy);	// 复制构造函数模板
...
template <class ElemType>
void DisplayBTWithTreeShape(const InThreadBinTree<ElemType> &bt);
	//	树状形式显示线索二叉树 
