...
#include "adj_matrix_undir_network.h"	// 邻接矩阵无向网
...
// 操作结果：返回w，使得边(w, adjVex[w])为连接V-U到U的具有最小权值的边
{
    ...
    for (v++; v < net.GetVexNum(); v++)	// 查找连接V-U到U的具有最小权值的边(w, adjVex[w])
		if (!net.GetTag(v) && net.GetWeight(v, adjVex[v]) >0 &&
...
{
	if (u0 < 0 || u0 >= net.GetVexNum())	throw "u0不合法1!";// 抛出异常
