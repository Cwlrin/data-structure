...
// 数据成员:
	ElemType data;								// 数据成员
	PostThreadBinTreeNode<ElemType> *leftChild;	// 左孩子指针成员
	PostThreadBinTreeNode<ElemType> *rightChild;// 右孩子指针成员
	PostThreadBinTreeNode<ElemType> *parent;	// 双亲指针成员
	PointerTagType leftTag, rightTag;			// 左右标成员成员

//	构造函数模板:
	PostThreadBinTreeNode();					// 无参数的构造函数 模板
	PostThreadBinTreeNode(const ElemType &val,	// 由数据元素值,指针及标志成员构造结点
...
// 操作结果：构造一个叶结点
{
	leftChild = rightChild = parent = NULL;		// 指针成员为空
	leftTag = rightTag = CHILD_PTR;				// 标志成员
}
...
PostThreadBinTreeNode<ElemType>::PostThreadBinTreeNode(const ElemType &val, 
		...
		PointerTagType lTag,
		PointerTagType rTag)
// 操作结果：构造一个数据成员为val,左孩子为lChild,右孩子为rChild的结点,双亲为
