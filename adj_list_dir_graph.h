...
#include "adj_list_graph_vex_node.h"			// 邻接表有向图顶点结点
...
template <class ElemType>
void Display(const AdjListDirGraph<ElemType> &g);					// 显示邻接矩阵有向图

// 有向图的邻接表类模板的实现部分
template <class ElemType>
void AdjListDirGraph<ElemType>::DFS(int v, void (*visit)(const ElemType &e)) const
// 初始条件：存在图
...
template <class ElemType>
void AdjListDirGraph<ElemType>::DFSTraverse(void (*visit)(const ElemType &e)) const
// 初始条件：存在图
// 操作结果：对图进行深度优先遍历
{
	int v;
...
template <class ElemType>
void AdjListDirGraph<ElemType>::BFS(int v, void (*visit)(const ElemType &)) const
// 初始条件：存在图
...
template <class ElemType>
bool AdjListDirGraph<ElemType>::GetTag(int v) const
// 操作结果：返回顶点v的标志		 
{
	if (v < 0 || v >= vexNum) throw "v不合法!";			// 抛出异常
...
template <class ElemType>
void AdjListDirGraph<ElemType>::SetTag(int v, bool val) const
// 操作结果：设置顶点v的标志为val		 
{
	if (v < 0 || v >= vexNum) throw "v不合法!";			// 抛出异常
...
template <class ElemType>
void Display(const AdjListDirGraph<ElemType> &g)
// 操作结果: 显示邻接矩阵有向图
{
	for (int v = 0; v < g.GetVexNum(); v++)
	{	// 显示第v个邻接链表
